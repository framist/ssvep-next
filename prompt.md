...
- [X] 加一个全局选项控制是否显示实验时间
- [ ] 分享优化：URL 实时变化。分享时弹出对话框，显示当前 URL，并提供复制按钮。同时显示 URL 二维码。URL 长度可否压缩？
...
- [x] **编辑项目 JSON**：在工具箱中添加一个"编辑项目 JSON"按钮，允许用户直接编辑当前项目的 JSON 配置文件。集成 AI 辅助功能，支持自定义 API 地址和模型名称，实现对画布内容的程序化、智能化编辑。
    - [ ] 优化整个的项目管理导入导出逻辑，减少冗余代码。（包括 `projectManager.ts`、`JsonEditorModal.tsx`、`useDemoSetup.ts`（可使用 JSON 格式的 Demo 示例）等）